...
class WorkspaceController extends Controller
{
    ...
    public function completeWorkspaceSetup(Request $request, $workspaceId)
    {
        ...
        $subscription = $workspace->subscriptions()->create([
            ...
            'plan' => $request->input('step4.selectedPlan'),
            'billing_cycle' => $request->input('step4.billingCycle', 'monthly'),
            'status' => 'active',
            'current_period_start' => now(),
            'current_period_end' => now()->addMonth(),
            'amount' => $this->calculatePlanAmount($request->input('step4.selectedPlan'), $request->input('step4.billingCycle', 'monthly'), count($request->input('step3.selectedFeatures'))),
            'currency' => 'usd',
            'quantity' => count($request->input('step3.selectedFeatures')),
            'features' => $request->input('step3.selectedFeatures'),
            'metadata' => [
                'selected_features' => $request->input('step3.selectedFeatures'),
                'billing_cycle' => $request->input('step4.billingCycle', 'monthly'),
